...
expr1 = mismatch_expression(model, simulated_data, m3, X, 10)

@test expr1 isa QuadExpr

@test_throws DimensionMismatch mismatch_expression(model, simulated_data, m3, X, 500) # too long X_len
simulated_data = [fill("A", 50); fill("B", 50)]
@test_throws DimensionMismatch mismatch_expression(model, simulated_data, m3, X, 10) # length of sim and X are not equal
